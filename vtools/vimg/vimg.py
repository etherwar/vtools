...
from .colors import * #WHITE, BLACK, RED, GREEN, BLUE, AQUA, MAROON, FUCHSIA, OLIVE, NAVY, TEAL, PURPLE, YELLOW
...
class vImg(np.ndarray):
    ...
    def __new__(cls, imgFn=None, **kwargs):
        ...
        def blank(height, width, clr=BLACK):
    ...
    def __array_wrap__(self, out_arr, context=None):
        ...
        self.__cDict = {'b': 'Blue', 'Green': 1, 'r': 'Red'}
    ...
    def pyramid(self, scale=1.5, minSize=(30, 30)):
        ...
        while True:
            ...
            # if the resized self does not meet the supplied minimum
            # size, then stop constructing the pyramid
            minX, minY = minSize
            if self.h < minX or self.w < minY:
                break

            # yield the next self in the pyramid
            yield image

    def slidingWindow(self, stepSize, windowSize):
        # slide a window across the image, yielding the current window
        yield next((x, y, self[y:y + windowSize[1], x:x + windowSize[0]])
    ...
    def show(self, title = None, wait = 0, waitKey = True):
        ...
        title   : Optional, string, represents an optionally provided custom title for the window 
        ...
        wait    : Optional, int (default is 0), represents the number of seconds provided to the 
                  cv2.waitKey function.
        waitKey : Optional, bool (default is True), set this parameter to False to disable invoking
